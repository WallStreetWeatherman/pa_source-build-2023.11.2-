...
def bjerksund_stensland(S, K, r, T, sigma, dividends, option_type="call"):
    ...
    def phi(S, T, gamma, H, I, r, sigma):
        ...
        try:
            some_log_value = math.log(S)  # Replace 'some_value' with the actual value you want to log
            print(f"Some log value: {some_log_value}")  # Debug
        except ValueError:
            print("Log domain error")  # Debug
    ...
    def phi_1(d, gamma, kappa):
        print(f"Inputs to phi1 - d: {d}, gamma: {gamma}, kappa: {kappa}")  # Debug
        cdf_d = stats.norm.cdf(d)
        print(f"CDF of d: {cdf_d}")  # Debug
        
        phi1_return_value = kappa * (cdf_d ** gamma)
        print(f"Phi1 return check: {phi1_return_value}")  # Debug
